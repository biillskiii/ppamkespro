...
const FormAjuan = () => {
  ...
  const checkSubmissionStatus = async (token) => {
    try {
      ...
      if (error.response && error.response.status === 409) {
        ...
        setSubmissionStatus("resubmit"); // Status 409 biasanya menunjukkan bahwa pengajuan sudah ada
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        // Arahkan ke halaman /view setelah pengajuan sukses
        router.push("/view");
      }
    } catch (error) {
      toast.error("Gagal mengirim pengajuan. Silakan coba lagi.");
  ...
  if (hasSubmitted && submissionStatus === "resubmit") {
    return (
      <div className="bg-gray-100 min-h-screen flex flex-col items-center justify-start">
